...
VoteSchema.virtual('topGG.hasVoted').get(function hasVoted() {
  return this.topGG.lastVoted == null ? false : moment(this.topGG.lastVoted).add(12, 'hours').isAfter(new Date());
});

VoteSchema.virtual('discordBotList.hasVoted').get(function hasVoted() {
  return this.discordBotList.lastVoted == null
    ? false
    : moment(this.discordBotList.lastVoted).add(12, 'hours').isAfter(new Date());
});

VoteSchema.virtual('rewardable').get(function hasVoted() {
  return this.lastReward == null ? true : moment(this.lastReward).add(12, 'hours').isBefore(new Date());
});
